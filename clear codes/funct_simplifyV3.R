...
simplify <- function(dataset, edge = "ind", n = 10, reduction.level=0.9, by=100){
  ...
  if      (edge=="ind"){
    ...
    for (i in 1:maxi){
      ...
      randomised_ind <- randomised_ind + 1
      
      # the "by" module
      if (randomised_ind == by){
    ...
    for (i in 1:maxi){
      ...
      randomised_int <- randomised_int + 1
      
      if (randomised_int == by){
        results[i+1, ord.cols] <- calcChar(as.matrix(temp.dat))
        randomised_int <- 0
      }
    }
